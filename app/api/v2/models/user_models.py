...
class PolitcianModel(UserModel):
    ...
    def retrieve_all_politicians(cls, office_id):
        ...
        query = """ SELECT concat_ws(' ', firstname, othername, lastname) AS candidate, user_id as related_userid, parties.party_name AS party, offices.office_name AS office, politicians.politician_id AS candidate_id, parties.party_id AS party_id, offices.office_id AS office_id
        ...
        politicians = []
        if records:
            column = ( 'candidate_name', 'related_userid', 'party', 'office', 'candidate_id', 'party_id', 'office_id')
